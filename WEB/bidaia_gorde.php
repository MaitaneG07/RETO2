...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $sql_check_tipo_viaje = "SELECT COUNT(*) FROM tipo_viaje WHERE id_tipo_viaje = ?";
    ...
    if (strtotime($fec_fin) <= strtotime($fec_ini)) {
        echo "<script>alert('La fecha de salida debe ser posterior a la fecha de entrada.');</script>";
    } else {
        $dias = (strtotime($fec_fin) - strtotime($fec_ini)) / (60 * 60 * 24);
        echo "<script>document.getElementById('dias').value = '$duracion';</script>";
    }
...
if ($conn->connect_error) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Viaje guardado correctamente.');</script>";
    } else {
        echo "<script>alert('Error al guardar el viaje: ');</script>" . $stmt->error;
    }
